...
fit2 <- arima(log_diff, order = c(16,0,0))
...
fit3 <- arima(log_diff, order = c(16,0,0))
...
puget %>%
  ...
  xlim(a, b) +
  ylim(200, 320) +
  scale_color_manual(name = 'Series', values = c('Actuals' = 'blue', '3 mo Avg' = 'red'))

# AR(16) model for full recursive view
fcasts4 <- vector(mode = "list", length = 24L)
for (i in 1:24) {# start recursive estimator
  log_diff = window(ld_puget, end = c(start - 1, 11 + i))
  fit4 <- arima(log_diff, order = c(16,0,0))
  fcasts4[[i]] <- forecast(fit4, h = 1)
  
}
# create 12 month forecast at end of series
fcasts4[[24]] <- forecast(fit4, h = 12)
