...
ld_ts <- diff(log(puget_ts))  # log difference of entire dataset
ld_est <- diff(log(est_sa_ts))  # log difference of estimation set
ld_pred <- diff(log(pred_sa_ts))  # log difference of prediction set
```
...
plot(ld_ts, col='black', main='Log Difference of \nSeasonally Adjusted trends', ylab='Seasonally Adjusted Index', xlab='Year')
...
acf(puget_ts, lag.max = 20, main='ACF Distribution')
...
pacf(puget_ts, lag.max = 20, main='PACF Distribution')
